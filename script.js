...
function renderTimeline(events, images) {
  ...
  events.forEach((event, index) => {
    const eventCard = document.createElement("a"); // Create a clickable link
    eventCard.className = `event ${index === 0 ? "active" : ""}`;
    eventCard.href = `https://en.wikipedia.org/wiki/${encodeURIComponent(event.title)}`; // Wikipedia page link
    eventCard.target = "_blank"; // Open link in a new tab
    eventCard.rel = "noopener noreferrer"; // Security for external links
